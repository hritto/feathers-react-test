...
import SoundLibrary_Tablet from './soundLibrary_tablet.js'

const media = function (options) {
  'use strict';
  ...
  var loadSoundEngine = function () {
    ...
    if (!getSoundLibrary() || is_android_Chrome()) {
      ...
      if (typeof window.AudioContext !== 'undefined' && canPlayWebApi()) { //Web Audio API
        ...
        sound_engine = new SoundLibrary();
      } else { // Sin Web Audio API
        sound_engine = new SoundLibrary_Tablet();
      }
      return sound_engine;
    } else {
  ...
  var loadMedia = function (asset_dirs, media_hash, callback_func) {
    ...
    promiseLoadImage('no_image', 'uploads/media/no_image.png', options).then(function () {
  ...
  var loadSoundByHtml5Audio = function (key, url) {
    var audio_url = "uploads/media/" + url;
