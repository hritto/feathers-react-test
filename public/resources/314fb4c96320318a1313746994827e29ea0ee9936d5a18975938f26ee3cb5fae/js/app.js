...
AnayaInfantil.App = function(opts, common_conf) {
  'use strict';
  ...
  var initialize = function() {
    ...
    resizer = AnayaInfantil.Resizer;
    scene_config = config.code[current_scene];
    layout = new AnayaInfantil.Layout(scene_config, common_config);
  ...
  var loadMedia = function(opts) {
    var asset_dirs = "images/activity/";
    media = new AnayaInfantil.media(opts);
    media.loadMedia(
      asset_dirs,
      opts.media,
  ...
  var initActivity = function() {
    //Marcar la actividad como abierta
    var level = localStorage.getItem(config.activity_key);
    if(level < 2){
      localStorage.setItem(config.activity_key, 1);
    }
    ...
    menu = new AnayaInfantil.Menu(common_config.menu_config, resizer, scene_config.instruction, common_config.alerts, common_config, scene_config, ok_index, ko_index);
    menu.init();
    menu.setMedia(media);
    scene = new AnayaInfantil["Scene_" + current_scene]();
  ...
  var endAllActivities = function(home_button) {
    console.log("terminadas todas");
    if(!home_button){
      //Marcar la actividad como terminada
      localStorage.setItem(config.activity_key, 2);
    }
    //Redirigir donde indica la configuración
    window.location = scenes[0].menu_url;
  };
  ...
  var promiseShowInfoWindow = function() {
    //CONFIGURACIÓN DE LA PANTALLA INICIAL (COMENZAR)
    var info_options = scenes[current_scene].info_config;
    var info_object = new AnayaInfantil.infoWindow(resizer);
    return new Promise(function(resolve, reject) {
      info_object.init(info_options).then(function() {
        info_object.destroy();
        info_object = null;
        return resolve();
      })
  ...
  return {
    ...
    promiseShowInfoWindow: promiseShowInfoWindow,
    getMenu: getMenu
  };
};
