...
AnayaInfantil.Scene_1 = function() {
  ...
  var checkClick = function(e) {
    ...
    if (resolution[e.target.id]) {
      ...
      */
      //mostrar la señal
      $('.check_ok').css({
        "width": resizer.getSimpleSize(30) + "px",
        "height": resizer.getSimpleSize(30) + "px",
        "top": resizer.getSimpleSize(0) + "px",
        "right": resizer.getSimpleSize(-20) + "px"
      });
	  
	  $('#' + e.target.id).css({
	  "border": "solid 4px #07fe07",
	  });
      if(animate_answers){
  ...
  var drawAnswers = function(els) {
	...
	//Desactivado para que no aparezcan en orden aleatorio
  ...
  var drawAnswerElement = function(el, k, total, container, index, pos) {
	...
	var elem_height = resizer.getSize(el.size).h;
